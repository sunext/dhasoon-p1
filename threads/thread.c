...
void thread_tick(void) {
	...
	//check if any sleeping threads, then wake them up now
	wake_sleeping_threads();

	// Test for thread max priority (alarm-priority test)
...
void thread_set_priority(int new_priority) {
	...
	/*
	 *  If a thread added to the ready list has higher priority than the running thread,
...
void check_thread_priority(void) {
	...
	/*if (thread_current()->priority < next_available_thread->priority) {
		(intr_context() == 1) ? intr_yield_on_return() : thread_yield();
	} else if (thread_current()->priority == next_available_thread->priority) {
		if (thread_current()->required_lock == NULL && next_available_thread->required_lock != NULL) {
			(intr_context() == 1) ? intr_yield_on_return() : thread_yield();
			return;
		}
		if (++thread_ticks >= TIME_SLICE) {
...
void wake_sleeping_threads() {
	...
	while (e != list_end(&sleep_list)) {
		...
		thread_wakeup(t); // change the state from sleeping to thread-ready

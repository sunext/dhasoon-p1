...
process_execute (const char *file_name)
{
  ...
  char *save_ptr;
  char *command = strtok_r(fn_copy, " ", &save_ptr);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (command, PRI_DEFAULT, start_process, file_name_copy);
  if (tid == TID_ERROR)
...
setup_stack (void **esp, char *file_name)
{
  ...
  char *token, *save_ptr;
  char **argv = malloc(DEF_ARR_CAPACITY);

  //Pushing the argv elements
  for (token = strtok_r (file_name, " ", &save_ptr); token != NULL;
	  ...
	  *esp -= size;
	  ...
	  if (argc >= argv_capacity) {
		  ...
		  argv_capacity += argv_capacity >> 1;
		  argv = realloc(argv, argv_capacity);
	  }
